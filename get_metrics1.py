...
def get_metrics_classification(y_test, y_pred, y_score, name):
    ...
    # Исправляем обращение к вероятностям классов
    if y_score.ndim == 2:  # Проверяем, что y_score двумерный (вероятности для каждого класса)
        df_metrics['ROC_AUC'] = roc_auc_score(y_test, y_score[:, 1]) # Берем вероятности положительного класса
        df_metrics['Logloss'] = log_loss(y_test, y_score)
    else: # Если y_score одномерный (вероятности только для одного класса)
